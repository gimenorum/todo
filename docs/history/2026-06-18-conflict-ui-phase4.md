...
- 再描画でのキャレット喪失回避: フォームは初回構築のみ・`update` は競合消滅時に完了表示へ切り替えるUI（`resolving` フラグで
  自分の確定中はフリップしない）。
- **画面文言は平易な日本語に**（一般ユーザー向け / IT 外来語を避ける）: 確定ボタン「マージを確定」→「**編集を確定**」、
  プレビュー見出し「マージ結果プレビュー」→「**編集後の内容**」、直接編集の選択肢「編集」→「**直接入力**」（全体の確定操作
  「編集」との語の重複回避）、本文「フィールド」→「**項目**」、空表示「未解決の競合」→「解決が必要な変更」。`マージ`/`競合`は
  ブランチ名・コミット・設計書などの内部表現には残す（画面に出る語のみ平易化）。
