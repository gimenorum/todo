...
## 追記（2026-06-18）PR #49 文言調整：「同期エラー」基調に統一

### 依頼内容
- 編集画面に追加した文言が「競合」を生で出しており、過去の文言方針と不整合との指摘。
- 過去ログ確認: `docs/history/2026-06-18-issue42-conflict-confirm.md` で「タイトルは『競合』『不具合』を避け
  『同期エラーを解決』とする」と決定済み。解決画面タイトルは既に「同期エラーを解決」だが、一覧の解決ボタンは
  「同期不具合を解決する」のまま不整合だった。
- ユーザー決定: 編集画面バナーを「同期エラー」基調に変更し、**一覧ボタンの不整合も今回あわせて是正**。

### 対応概要
- `src/ui/views/TodoEditView.ts`:
  - ボタン「競合を解決」→ **「同期エラーを解決」**（解決画面タイトルと統一）。
  - バナー文「このタスクには競合があります。…」→ **「このタスクは同期エラーがあります。編集前に解決してください。」**。
- `index.html`: 一覧の解決ボタン（`todo-resolve`）「同期不具合を解決する」→ **「同期エラーを解決する」**（不整合是正）。
- `styles/components.css`: バナーのコメントを「同期エラー」基調に微修正（クラス名 `.edit-conflict-note` は内部名として据え置き）。

### 決定事項
- ユーザー向け UI 文言は「競合」「不具合」を避け **「同期エラー」基調**で統一（Issue #42 方針を一覧ボタンにも適用）。
- 「競合（conflict）」はコード・設計の**内部用語**としては引き続き使用（クラス名・型・コメント）。
- 履歴は上書きせず本節として追記。

### 成果物（追記分）
- 変更: `src/ui/views/TodoEditView.ts`
- 変更: `index.html`
- 変更: `styles/components.css`（コメントのみ）
