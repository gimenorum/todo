...
## 追記（2026-06-19）案② 横並び（2カラム）に決定（モックアップ提示 → 選択）

### 経緯
- フル幅統一でも「修正が一周している（＝当初 #48 のフル幅 NG に逆戻り）」との指摘。狭幅・フル幅いずれも NG で
  堂々巡りになったため、**修正前にモックアップ画像（3案）を提示**して方向を決定する運用に切替（オーナー要望）。
- 3案（①フル幅／②横並び2カラム／③中程度の固定幅）を画像提示 → **案②（横並び2カラム）** を選択。

### 対応概要
- `src/ui/views/TodoEditView.ts`: 期日（`.f-due`）と優先度（`.f-priority`）の各 `.field` を新コンテナ
  `.field-row` で囲み、1 行に横並び（2 カラム）にする。`form.append` の並びを更新。
- `styles/components.css`: `.field-row { display:flex; gap:12px }` ＋ `.field-row > .field { flex:1; min-width:0 }`。
  各カラム内の入力欄は共有ルールの `width:100%` で満たす。期日値の左寄せ（`::-webkit-date-and-time-value`）は維持、
  select のカスタム矢印も維持。
- CSS＋DOM のみ・データ/挙動は不変。version 0.4.10 のまま、同ブランチに追加コミット。

### 決定事項
- #48 の最終形は **「期日・優先度を 1 行に横並び（2カラム・半幅）」**。フル幅・狭幅は不採用。
- 今後の UI 変更は、着手前にモックアップ画像で合意を取ってから実装する。
